(function () {
  ...
  function describeAxis(value) {
    ...
    return [
      value.axis_id || value.axis || value.name,
      value.question,
      visibleList(value.covered_studies),
      visibleList(value.missing_units),
      visibleList(value.incomparable_items),
      visibleList(value.counterevidence_ids),
      value.impact_on_conclusion,
    ].filter(Boolean).join(" · ") || "比较轴待核对";
  }
  function decisionLine(value) {
    return `决定：${value.action}；执行者：${value.actor_type || "—"} / ${value.actor_label || "—"}；理由：${value.reason || "—"}`;
  }
  ...
  function render(protocol, synthesis, contracts, figures) {
    ...
    (figures.source_figures || []).forEach(item => {
      ...
      details.append(text("strong", publication.title || "论文身份待核对"));
      ...
      details.append(text("p", `出处：${item.attribution || "—"}`));
      details.append(text("p", `权利状态：${item.rights_context?.status || "unknown"}；${item.rights_context?.notice || "Publication reuse rights are not cleared."}`, "decision-line"));
    ...
    (figures.placeholders || []).forEach(item => figurePanel.append(text("p", `任务：${item.scientific_question || "—"}；读者结论：${item.reader_takeaway || "—"}；缺口原因：${item.gap_reason || "—"}；状态：${item.status || "—"}`, "figure-placeholder-row")));
