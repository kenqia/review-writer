...
class NativeReviewWriterPluginTests(unittest.TestCase):
    ...
    def test_quickstart_makes_cli_primary_and_parks_qoderwork(self) -> None:
        quickstart = (ROOT / "docs/qoderwork/research_review_writer_quickstart.md").read_text(encoding="utf-8")

        self.assertIn("QoderWork 入口说明（停车状态）", quickstart)
        self.assertIn("当前用户主流程是仓库内的确定性 CLI", quickstart)
        for command in (
            "bootstrap-corpus",
            "bind-generic-parse",
        ...
        self.assertIn("对用户意味着什么", quickstart)
        self.assertIn("维护者保留内容", quickstart)
        self.assertIn("不能宣称“用户打开插件即可得到已审阅综述”", quickstart)
        for obsolete_user_flow in (
            "确认 Review Brief",
            "上传一个 ZIP",
            "审阅 Scientific Risk Packet",
            "编辑并下载最终 DOCX",
            "模型选择 Qwen3.7-Max",
        ):
            self.assertNotIn(obsolete_user_flow, quickstart)
