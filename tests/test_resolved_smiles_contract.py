...
def test_parent_v2_optional_element_review_loads_but_stales_material_claim(
    tmp_path: Path,
) -> None:
    project = _project_with_molecules(
        tmp_path, [_molecule("mol-a", expanded="CO")]
    ...
    review_chemical_paper_elements(
        ...
        actor=reviewer,
        reason="Optional element review before resolution.",
        version_token=chemical_paper_projection(project)["studies"][0][
            "version_token"
        ],
    )

    # Re-seal the exact event shape generated by parent 04438a7: v2 did not
    # emit bound_resolution_event_digest for an optional pre-resolution review.
    state = json.loads(state_path.read_text(encoding="utf-8"))
    legacy_review = state["element_reviews"][0]
    legacy_review.pop("bound_resolution_event_digest")
    legacy_review["event_digest"] = canonical_digest(
        {key: value for key, value in legacy_review.items() if key != "event_digest"}
    )
    state["element_review_head_digest"] = legacy_review["event_digest"]
    _reseal_state(state_path, state)

    before_load = state_path.read_bytes()
    loaded = load_chemical_paper_state(project, "scholarly-a")
    assert "bound_resolution_event_digest" not in loaded["element_reviews"][0]
    assert state_path.read_bytes() == before_load
    ...
    binding = chemical_paper_manuscript_bindings(project)
    before_currentness = state_path.read_bytes()
    ...
    assert state_path.read_bytes() == before_currentness
