...
def main() -> int:
    ...
    issues = [] if actual == {"results.jsonl"} else ["output must contain exactly results.jsonl before finalization"]
    semantic_issues, stats = validate_results(root, results_path)
    ...
    output_manifest = {
        ...
        "upstream_layer_a_results_sha256": input_manifest["upstream_layer_a_results_sha256"],
        "upstream_layer_a_input_manifest_hash": input_manifest["upstream_layer_a_input_manifest_hash"],
        "repo_head": input_manifest["repo_head"],
        "results_sha256": sha256_file(results_path),
        **stats,
        "status": "PASS",
    }
    atomic_write(manifest_path, json.dumps(output_manifest, ensure_ascii=True, indent=2, sort_keys=True) + "\n")
    atomic_write(checksum_path, f"{sha256_file(manifest_path)}  OUTPUT_MANIFEST.json\n{sha256_file(results_path)}  results.jsonl\n")
