...
仓库中的 `review_writer.agent.qoderwork_adapter`，并把所有科学判断交给现有 Dashboard。
...
VersionContext 节点和 Dashboard 健康检查，然后通常返回 `HUMAN_ACTION_REQUIRED`。把返回的
真实 loopback Dashboard URL 展示给用户并停止。
