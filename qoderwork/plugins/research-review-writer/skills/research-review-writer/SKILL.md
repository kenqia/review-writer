...
- 发布 queue 后，同一个任务观察固定的项目相对 inbox：`review-projects/<project_id>/00_sources/manual_upload/inbox/source_bundle.zip`。文件出现后只运行一次 `scripts/acquisition/import_manual_archive.py`，随后立即运行 `scripts/acquisition/acquire_public_corpus.py` 的 `--verify-only` 并继续既有 MinerU 阶段；不要求研究者再次点击继续，也不询问或接受 ZIP 路径。unmatched / ambiguous / missing rows 继续留在同一个 queue，绝不做模糊、内容或模型匹配。
