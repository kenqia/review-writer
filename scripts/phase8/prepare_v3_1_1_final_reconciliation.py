...
from review_writer.phase8.v3_1_1_reconciliation import prepare_final_reconciliation  # noqa: E402
...
LAYER_B_REPO_HEAD = "765970ccc2a6ccccec2236fcb4d8569aba6a87a1"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare the final V3.1.1 A/B reconciliation and four human spot checks.")
    parser.add_argument("--repo-root", type=Path, default=REPO_ROOT)
    parser.add_argument("--workspace-parent", type=Path, default=WORKSPACE_PARENT)
    parser.add_argument("--layer-a-workspace", type=Path, default=None)
    parser.add_argument("--layer-b-workspace", type=Path, default=None)
...
def require_pushed_clean_head(repo_root: Path) -> str:
    if git_value(repo_root, "status", "--porcelain"):
        raise ValueError("reconciliation generation requires a clean committed worktree")
    head = git_value(repo_root, "rev-parse", "HEAD")
    upstream = git_value(repo_root, "rev-parse", "@{upstream}")
    if head != upstream:
        raise ValueError("reconciliation generation requires HEAD to equal its pushed upstream")
    return head


def run_id_now() -> str:
    return time.strftime("phase8_final_reconciliation_v3_1_1_%Y%m%dT%H%M%SZ", time.gmtime())
...
def main() -> int:
    ...
    try:
        ...
        layer_a = args.layer_a_workspace or workspace_parent / LAYER_A_RUN_ID / "layerA_inventory"
        layer_b = args.layer_b_workspace or workspace_parent / LAYER_B_RUN_ID / "layerB_exact_claim_verifier"
        result = prepare_final_reconciliation(
            repo_root=repo_root,
            output_parent=workspace_parent,
            run_id=args.run_id or run_id_now(),
            layer_a_workspace=layer_a,
            layer_b_workspace=layer_b,
            ...
            expected_layer_b_repo_head=LAYER_B_REPO_HEAD,
        ...
        print(f"phase8-v3.1.1-final-reconciliation: {exc}", file=sys.stderr)
        return 1
