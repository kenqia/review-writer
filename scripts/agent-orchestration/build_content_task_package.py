#!/usr/bin/env python3
"""Build a bounded Content Agent package manifest."""

from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

REPO_ROOT = Path(__file__).resolve().parents[2]
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from review_writer.project.content_agent_handoff import (  # noqa: E402
    ContentAgentError,
    build_content_task_package,
)


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--project", type=Path, required=True)
    parser.add_argument("--request", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path)
    args = parser.parse_args()
    try:
        request = json.loads(args.request.read_text(encoding="utf-8"))
        package = build_content_task_package(args.project, request, args.output_dir)
    except (OSError, ValueError, json.JSONDecodeError, ContentAgentError) as exc:
        print(getattr(exc, "code", str(exc)), file=sys.stderr)
        return 1
    print(json.dumps(package, ensure_ascii=False, sort_keys=True, indent=2))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
