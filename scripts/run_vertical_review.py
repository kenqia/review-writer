...
from review_writer.project.input_provenance import (  # noqa: E402
...
def _parser() -> argparse.ArgumentParser:
    ...
    corpus_bootstrap = commands.add_parser(
        "bootstrap-corpus",
        help="Create the authoritative variable-N 20–40 study source-only project",
    )
    corpus_bootstrap.add_argument("--review-root", type=Path, required=True)
    corpus_bootstrap.add_argument("--request", type=Path, required=True)
    ...
    corpus_preflight = commands.add_parser(
        ...
        help="只读检查 20–40 篇项目的 MAIN/SI/Generic/Chemical 输入绑定",
    )
    corpus_preflight.add_argument("--project", type=Path, required=True)
    corpus_preflight.add_argument("--manifest", type=Path, required=True)

    corpus_import = commands.add_parser(
        "import-corpus-inputs",
        help="发布已通过检查的 20–40 篇输入 provenance；不等于科学确认",
    )
    corpus_import.add_argument("--project", type=Path, required=True)
    corpus_import.add_argument("--manifest", type=Path, required=True)
...
def _run(args: argparse.Namespace) -> int:
    if args.command == "bootstrap-dual-parse":
        ...
        project = bootstrap_dual_parse_project(args.review_root, request)
